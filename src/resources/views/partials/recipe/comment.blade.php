    ...
    <div class="row g-0 p-3">
        ...
        @if(Gate::inspect('create', $comment)->allowed())
            <div class="col-md text-end position-relative">
                <button data-role="a" class="btn btn-sm btn-outline-secondary p-1 m-1 recipe-comment-reply-button" data-toggle="tooltip" data-placement="top" title="Reply to this comment"><i class="fas fa-reply"></i> Reply</button>
            </div>
        @endif
        @if(Gate::inspect('update', $comment)->allowed())
            <div class="col-md text-end position-relative">
                <button data-role="a" class="btn btn-sm btn-outline-secondary p-1 m-1 recipe-comment-edit-button" data-toggle="tooltip" data-placement="top" title="Edit your comment"><i class="fas fa-edit me-1"></i>Edit</button>
            </div>
        @endif
        @if(Gate::inspect('delete', $comment)->allowed())
            <div class="col-md text-end position-relative">
                <button data-role="a" class="btn btn-sm btn-outline-secondary p-1 m-1 recipe-comment-delete-button" data-toggle="tooltip" data-placement="top" title="Delete your comment"><i class="fas fa-trash-alt"></i> Delete</button>
            </div>
        @endif
        </div>
    </div>
