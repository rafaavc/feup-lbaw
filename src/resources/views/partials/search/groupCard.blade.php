<div class="search-card">
    <a type="button" href="{{ url('/group/' . $group->id) }}" class="btn card shadow-sm p-2">
        ...
        <div class="card-body">
            ...
            <p class="text-muted m-0 card-info"><span class="info-number">{{ $group->members->count() }}</span> recipes</p>
            <p class="text-muted m-0 mt-1 card-info"><span class="info-number">{{ $group->recipes->count() }}</span>  members</p>
        </div>
    </a>
</div>
